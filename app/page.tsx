...
function HomeContent(){
  ...
  const[mode,setMode]=useState<"create"|"join">("create"),[name,setName]=useState(""),[joinCode,setJoinCode]=useState(""),[session,setSession]=useState<Session|null>(null),[game,setGame]=useState<GameState|null>(null),[busy,setBusy]=useState(false),[syncing,setSyncing]=useState(true),[error,setError]=useState(""),[copied,setCopied]=useState(false),[guesses,setGuesses]=useState<string[]>([]),[account,setAccount]=useState<Account|null>(null),[viewer,setViewer]=useState<ViewerIdentity|null>(null),[entryMode,setEntryMode]=useState<EntryMode>(ACCOUNTS_ENABLED?null:"guest"),[accountLoading,setAccountLoading]=useState(ACCOUNTS_ENABLED),[accessToken,setAccessToken]=useState("");
  ...
  useEffect(()=>{
    if(!ACCOUNTS_ENABLED){setAccountLoading(false);return}
  ...
  async function signOut(){try{await getSupabaseBrowserClient().auth.signOut()}finally{setAccessToken("");setViewer(null);setAccount(null);setEntryMode(ACCOUNTS_ENABLED?null:"guest");setName("");setError("")}}
    ...
    <section className="relative isolate overflow-hidden border-b border-white/10">
      ...
      <div className="mx-auto grid min-h-[calc(100vh-65px)] max-w-6xl items-center gap-8 px-5 py-10 sm:py-14 lg:grid-cols-[1.05fr_.95fr] lg:gap-12">
        ...
        <Card id="play" className="landing-card scroll-mt-24 border-white/15"><CardContent className="pt-6">
          <div className="mb-5 flex items-center justify-between gap-3"><div><p className="label mb-2">{t("Enter the investigation")}</p><h2 className="font-serif text-3xl">{t(ACCOUNTS_ENABLED&&entryMode===null?"Account or guest":ACCOUNTS_ENABLED&&entryMode==="account"&&!account?"Account access":mode==="create"?"Open a private room":"Use your invitation code")}</h2></div><LanguageToggle/></div>
          {ACCOUNTS_ENABLED&&entryMode===null?<div><p className="mb-4 text-sm leading-6 text-slate-400">{t("Choose how you enter")}</p><div className="grid gap-3">
          ...
          </div></div>:ACCOUNTS_ENABLED&&entryMode==="account"&&!account?<AccountAccess viewer={viewer} saveProfile={saveProfile} authenticated={authenticated} back={changeEntry}/>:ready?<>
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-white/10 bg-black/20 p-3"><span className={`grid size-10 shrink-0 place-items-center rounded-full ${entryMode==="account"?"bg-amber-100/10 text-amber-100":"bg-sky-100/10 text-sky-200"}`}>{entryMode==="account"?<UserRound/>:<Smartphone/>}</span><div className="min-w-0 flex-1"><p className="text-xs uppercase tracking-[.14em] text-slate-500">{t("Playing as")}</p><p className="truncate text-sm text-stone-200">{entryMode==="account"?account?.username:t("Temporary guest")}</p></div>{ACCOUNTS_ENABLED&&<Button type="button" variant="ghost" size="sm" onClick={changeEntry}>{t("Change")}</Button>}{ACCOUNTS_ENABLED&&entryMode==="account"&&<Button type="button" variant="ghost" size="icon" onClick={()=>void signOut()} aria-label={t("Log out")} title={t("Log out")}><LogOut/></Button>}</div>
