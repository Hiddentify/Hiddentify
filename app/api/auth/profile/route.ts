...
import { isUniqueViolation, logServerError } from "@/lib/server-log";
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(isUniqueViolation(error))return Response.json({error:sq?"Ky email ose username po përdoret tashmë.":"That email or username is already in use."},{status:409,headers:noStoreHeaders});
    logServerError("account_profile_save_failed",error,request);
